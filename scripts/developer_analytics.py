...
def validate_config(config: dict[str, Any]) -> None:
    ...
    for index, project in enumerate(projects):
        ...
        for evidence in project.get("evidence", []):
            if (
                ...
                or not evidence.get("label")
                or not str(evidence.get("url", "")).startswith("https://github.com/")
            ):
    ...
    for assignment in config.get("assignment_profiles", []):
        ...
        for signal in signals:
            ...
            if category not in {"technology", "domain", "capability", "practice", "ownership"}:
                raise ConfigError(f"{assignment_id}: invalid category {category!r}")
...
class GitHubClient:
    """Small GitHub REST client that intentionally avoids Search API rate limits."""
    ...
    def get(self, path: str, params: dict[str, Any] | None = None) -> Any:
        ...
        headers = {
            ...
            "User-Agent": "mizzz-developer-analytics/1.1",
    ...
    def pages(self, path: str, params: dict[str, Any] | None = None, *, max_pages: int = 3) -> list[dict[str, Any]]:
        base = dict(params or {})
        base["per_page"] = 100
        ...
        for page in range(1, max_pages + 1):
            ...
            if not isinstance(payload, list):
                raise GitHubAPIError(f"GET {path}: expected list response")
    ...
    def tree(self, repo: str, branch: str) -> list[str]:
        result = self.get(
            f"/repos/{repo}/git/trees/{urllib.parse.quote(branch, safe='')}",
            {"recursive": 1},
        )
        return [
            str(item["path"])
    ...
    def contributions(self, repo: str, login: str) -> dict[str, int]:
        ...
        authored_pulls = [
            ...
            if str((pull.get("user") or {}).get("login") or "").lower() == login_lower
        ]

        issues = self.pages(
            f"/repos/{repo}/issues",
            {"state": "all", "creator": login, "sort": "updated", "direction": "desc"},
            max_pages=3,
        )
        authored_issues = [issue for issue in issues if not issue.get("pull_request")]
        return {
            "prs_opened": len(authored_pulls),
            "prs_merged": sum(1 for pull in authored_pulls if pull.get("merged_at")),
            "issues_opened": len(authored_issues),
            "issues_closed": sum(1 for issue in authored_issues if issue.get("state") == "closed"),
        }
...
def detect_delivery_signals(paths: Iterable[str]) -> dict[str, bool]:
    ...
    def contains(*patterns: str) -> bool:
        return any(any(pattern in path for pattern in patterns) for path in lowered)

    return {
        "ci_cd": any(path.startswith(".github/workflows/") for path in lowered),
        "tests": contains("tests/", "test/", "__tests__", ".test.", ".spec.", "pytest.ini", "vitest", "playwright"),
        "documentation": any(path == "readme.md" or path.startswith("docs/") for path in lowered),
...
def collect_project(client: GitHubClient, configured: dict[str, Any], login: str, now: datetime) -> dict[str, Any]:
    ...
    contributions = client.contributions(repo, login)

    contribution_points = min(
        25,
        contributions["prs_merged"] * 2
        + contributions["prs_opened"]
        + contributions["issues_closed"]
        + contributions["issues_opened"] // 2,
    )
    ...
    return {
        ...
        "contribution_scope": "up-to-300-recent-repository-items",
...
def collect_project_offline(configured: dict[str, Any]) -> dict[str, Any]:
    ...
    score = min(
        100,
        20
        + len(configured.get("technologies", [])) * 2
        + len(practices) * 2
        + len(configured.get("evidence", [])) * 4,
    )
    return {
        "repo": normalize_repo(configured["repo"]),
        "title": configured.get("title") or configured["repo"].split("/")[-1],
        "url": f"https://github.com/{configured['repo']}",
        "description": configured.get("summary") or "",
        "featured": bool(configured.get("featured")),
        "archived": False,
        ...
        "capabilities": sorted(set(configured.get("capabilities", []))),
        "practices": practices,
        "configured_practices": practices,
        "detected_practices": [],
        "ownership": sorted(set(configured.get("ownership", []))),
        "delivery_signals": {},
        "contributions": {"prs_opened": 0, "prs_merged": 0, "issues_opened": 0, "issues_closed": 0},
        "contribution_scope": "not-collected",
        "last_evidence_at": None,
        "recency_days": None,
        "recency_source": "not_collected",
        "evidence_score": score,
        "evidence_level": level(score),
        "evidence": configured.get("evidence", []),
    }
...
def aggregate(projects: list[dict[str, Any]], key: str) -> list[dict[str, Any]]:
    ...
    for name, items in grouped.items():
        recency = min(
            (item["recency_days"] for item in items if item.get("recency_days") is not None),
            default=None,
        )
        ...
        rows.append(
            {
                ...
                "projects": sorted(item["repo"] for item in items),
                "last_evidence_at": latest.isoformat().replace("+00:00", "Z") if latest else None,
                "recency_days": recency,
                "merged_prs": merged,
...
def assignment_fit(config: dict[str, Any], projects: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    available = {
        category: {str(value).lower() for project in projects for value in project.get(key, [])}
    ...
    for assignment in config.get("assignment_profiles", []):
        ...
        rows.append(
            {
                "id": assignment["id"],
                "label": assignment.get("label") or assignment["id"],
...
def build_analytics(
    ...
    *,
    ...
    before = {
        row.get("name"): row.get("score", 0)
        for row in (previous or {}).get("skills", [])
        if row.get("name")
    }
    changed = [
        {"name": name, "from": before[name], "to": score, "delta": score - before[name]}
        for name, score in current.items()
        if name in before and before[name] != score
    ]
    changed.sort(key=lambda item: (-abs(item["delta"]), item["name"].lower()))

    summary = {
        "tracked_projects": len(projects),
        "active_projects_90d": sum(
            1 for project in projects
            if project.get("recency_days") is not None and project["recency_days"] <= 90
        ),
        "technologies": len(dimensions["skills"]),
        "domains": len(dimensions["domains"]),
        "capabilities": len(dimensions["capabilities"]),
        "practices": len(dimensions["practices"]),
        "merged_prs_observed": sum(project["contributions"]["prs_merged"] for project in projects),
        "closed_issues_observed": sum(project["contributions"]["issues_closed"] for project in projects),
    }

    return {
        "schema_version": SCHEMA_VERSION,
        "generated_at": generated_at.isoformat().replace("+00:00", "Z"),
        "scope": "public-github-evidence",
        "collection_mode": collection_mode,
        "profile": config["profile"],
        "summary": summary,
        "projects": sorted(
            projects,
            key=lambda project: (-int(project["featured"]), -project["evidence_score"], project["repo"].lower()),
        ),
        **dimensions,
        "assignment_fit": assignment_fit(config, projects),
        "trend": {
            "previous_snapshot_at": (previous or {}).get("generated_at"),
            "new_skills": sorted(set(current) - set(before)) if previous else [],
            "changed_skills": changed[:15],
        },
        "data_quality": {
            "errors": errors or [],
            "limitations": [
                ...
                "PR and issue counts are bounded observations from up to 300 recent repository items per endpoint.",
...
def render_readme_block(data: dict[str, Any]) -> str:
    ...
    domains = data["domains"][:6]
    practices = data["practices"][:8]
    fits = data["assignment_fit"][:5]
    stamp = data["generated_at"].replace("T", " ").replace("Z", " UTC")
    if data["collection_mode"] == "github-api":
        note = f"Manual Developer Analytics snapshot · {stamp} · public evidence only"
    else:
        note = f"Bootstrap snapshot · {stamp} · curated public evidence; live recency/PR counts are collected on manual run"
    return "\n".join(
        [
            README_START,
            ...
            "**Evidence Scoreは能力点ではなく、公開GitHub上で確認できる証拠の強さです。**",
            "",
            "### Core technology signals",
            "",
            table(
                ["Technology", "Evidence", "Projects", "Latest"],
                [[row["name"], row["level"], row["project_count"], recency(row)] for row in skills],
            ),
            "",
            "### Engineering range",
            "",
            " ".join(f"`{row['name']}`" for row in domains),
            "",
            "### How I ship",
            "",
            " ".join(f"`{row['name']}`" for row in practices),
            "",
            "### Assignment fit // Evidence coverage",
            "",
            table(
                ["Area", "Coverage", "Matched", "Gap"],
                [[row["label"], f"{row['score']}%", len(row["matched"]), len(row["missing"])] for row in fits],
            ),
            "",
            f'<p align="center"><sub>{note} · <a href="./SKILL_SHEET.md">SKILL_SHEET.md</a> · <a href="./reports/developer-analytics.md">Detailed analytics</a></sub></p>',
            README_END,
        ]
    )
...
def render_report(data: dict[str, Any]) -> str:
    ...
    lines = [
        "# Developer Analytics",
        "",
        f"Generated: `{data['generated_at']}` · mode: `{data['collection_mode']}`",
        "",
        "> Public GitHub evidence only. Not a proficiency, years-of-experience, or productivity score.",
        "",
        "## Coverage",
        "",
        table(
            ["Metric", "Value"],
            [
                ["Tracked public projects", summary["tracked_projects"]],
                ["Active projects (90d)", summary["active_projects_90d"]],
                ["Technologies", summary["technologies"]],
                ["Domains", summary["domains"]],
                ["Capabilities", summary["capabilities"]],
                ["Engineering practices", summary["practices"]],
                ["Merged PRs observed", summary["merged_prs_observed"]],
                ["Closed issues observed", summary["closed_issues_observed"]],
            ],
        ),
        "",
    ]
    for heading, key in (
        ("Technology evidence", "skills"),
        ("Engineering practices", "practices"),
        ("Delivery ownership", "ownership"),
    ):
        lines += [
            f"## {heading}",
            "",
            table(
                ["Signal", "Score", "Evidence", "Projects", "Latest"],
                [[row["name"], row["score"], row["level"], row["project_count"], recency(row)] for row in data[key]],
            ),
            "",
        ]
    lines += [
        ...
        "",
        table(
            ["Assignment", "Coverage", "Matched", "Missing"],
            [
                [
                    row["label"],
                    f"{row['score']}%",
                    ", ".join(item["value"] for item in row["matched"]),
                    ", ".join(item["value"] for item in row["missing"]) or "-",
                ]
                for row in data["assignment_fit"]
            ],
        ),
        "",
        "## Project evidence",
        "",
        table(
            ["Project", "Evidence", "Recency", "Merged PRs", "Issues done", "Signals"],
            [
                [
                    f"[{project['repo']}]({project['url']})",
                    f"{project['evidence_score']} / {project['evidence_level']}",
                    f"{project['recency_days']}d" if project.get("recency_days") is not None else "not observed",
                    project["contributions"]["prs_merged"],
                    project["contributions"]["issues_closed"],
                    ", ".join(project["detected_practices"][:6] or project["configured_practices"][:6]),
                ]
        ...
        "",
        *(f"- {item}" for item in data["data_quality"]["limitations"]),
    ]
...
def render_skill_sheet(data: dict[str, Any]) -> str:
    ...
    fits = data["assignment_fit"]
    projects = [project for project in data["projects"] if project.get("featured")] or data["projects"][:6]
    strong = [row for row in skills if row["level"] in {"STRONG", "REPEATED"}][:12] or skills[:10]

    lines = [
        "# Skill Sheet — いゔる。 / mizzz",
        "",
        f"> 生成日時: `{data['generated_at']}` / Source: Public GitHub Evidence / Mode: `{data['collection_mode']}`  ",
        "> フリーランス・受託・案件アサイン時に再利用できる公開Evidenceベースのスキルシートです。",
        "",
        "## 基本情報",
        "",
        f"- **Name / Handle:** {profile.get('display_name', '')}",
        f"- **GitHub:** https://github.com/{profile['github_login']}",
        f"- **Website:** {profile.get('website', '')}",
        f"- **Contact:** {profile.get('contact', '')}",
        f"- **Role:** {profile.get('headline', '')}",
        "",
        "## エンジニアリング概要",
        "",
        profile.get("summary", ""),
        ...
        "### 公開Evidenceが特に強い技術",
        "",
        " / ".join(f"**{row['name']}**" for row in strong),
        "",
        "## 技術Evidence",
        "",
        table(
            ["Technology", "Evidence", "Score", "Projects", "Latest public evidence"],
            [[row["name"], row["level"], row["score"], row["project_count"], recency(row)] for row in skills[:20]],
        ),
        "",
        ...
        table(
            ...
            [[row["name"], row["level"], ", ".join(row["projects"][:5])] for row in practices[:18]],
        ),
        "",
        ...
        table(
            ...
            [[row["name"], row["level"], ", ".join(row["projects"][:5])] for row in ownership[:14]],
        ),
        "",
        ...
        table(
            ...
            [
                [
                    row["label"],
                    f"{row['score']}%",
                    ", ".join(item["value"] for item in row["missing"]) or "Configured signal setでは不足なし",
                ]
                for row in fits
            ],
        ),
    ...
    for project in projects:
        lines += [
            ...
            "",
            project["description"] or "Public project evidence.",
            "",
            f"- **Domains:** {', '.join(project['domains']) or '-'}",
            f"- **Technologies:** {', '.join(project['technologies']) or '-'}",
            f"- **Capabilities:** {', '.join(project['capabilities']) or '-'}",
            f"- **Ownership:** {', '.join(project['ownership']) or '-'}",
            f"- **Observed delivery / practices:** {', '.join(project['practices']) or '-'}",
        ]
        lines += [
            f"- **Evidence:** [{evidence['label']}]({evidence['url']})"
            for evidence in project.get("evidence", [])[:4]
        ]
        lines.append("")
    ...
    lines += [
        ...
        "- PR / Issue件数は各Repositoryの最近の最大300件を観測対象にするため、厳密なlifetime totalsではありません。",
...
def write_outputs(
    ...
    *,
    ...
    if readme:
        updated = replace_readme_block(readme.read_text(encoding="utf-8"), render_readme_block(data))
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.validate_only:
        print(
            f"Validated {len(config['projects'])} public analytics projects and "
            f"{len(config.get('assignment_profiles', []))} assignment profiles"
        )
        return 0
    ...
    if args.offline:
        ...
        login = config["profile"]["github_login"]
    ...
    write_outputs(
        ...
        output=args.output,
        report=args.report,
        skill_sheet=args.skill_sheet,
        readme=args.readme,
        snapshot_dir=None if args.no_snapshot else args.snapshot_dir,
    )
    print(
        f"Generated Developer Analytics: projects={len(projects)} "
        f"skills={len(data['skills'])} practices={len(data['practices'])} "
        f"assignments={len(data['assignment_fit'])}"
    )
    return 0


def parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate public evidence-backed Developer Analytics and Markdown skill sheet")
    parser.add_argument("--config", type=Path, default=Path(".github/developer-analytics.yml"))
    parser.add_argument("--output", type=Path, default=Path("data/developer-analytics/latest.json"))
    parser.add_argument("--report", type=Path, default=Path("reports/developer-analytics.md"))
    parser.add_argument("--skill-sheet", type=Path, default=Path("SKILL_SHEET.md"))
    parser.add_argument("--readme", type=Path, default=Path("README.md"))
    parser.add_argument("--snapshot-dir", type=Path, default=Path("data/developer-analytics/snapshots"))
